...
$(document).ready(function () {
	...
	$('[data-role="selects"]').on('change', '[type="radio"]', function (event) {
		...
		switch (element) {
			case'other_dfs':
				$('#other_date_format').removeAttr('readonly');
				break;
			case'other_tfs':
				$('#other_time_format').removeAttr('readonly');
				break;
			default:
				$('[data-id="other"]').attr('readonly', 'readonly');
				break;
		}
	});
	...
	$('[data-format-id="other"]').on('input', '[type="text"]', function (event) {
		...
		switch (element) {
			case'other_date_format':
				$('#other_dfs').val(element_value);
				break;
			case'other_time_format':
				$('#other_tfs').val(element_value);
				break;
			default:
				break;
		}
