$(function () {
	...
	$('body').on('click', '.panel-blocks #disable, .panel-blocks #enable', function () {
		...
		if (list.length <= 0) {
			return mcr.notify(lng.error, lng_block.not_selected);
		}
		...
		$.ajax({
			url: "index.php?mode=ajax&do=admin_blocks_status",
			dataType: "json",
			type: "POST",
			async: true,
			cache: false,
			...
			error: function (data) {
				mcr.logger(data);
				mcr.notify(lng.error, lng_block.e_change_status);
			},
