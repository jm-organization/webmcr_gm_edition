...
class submodule{
	...
	public function content(){
		...
		if (!$this->core->is_access('sys_comment_edt')
		    && !$this->core->is_access('sys_comment_edt_all')) {
			$this->core->js_notify($this->lng['com_perm_edit']);
		}
		...
		if (!$this->is_discus($nid)) {
			$this->core->js_notify($this->lng['com_disabled']);
		}
		...
		if (empty($message)) { $this->core->js_notify($this->lng['com_msg_empty']); }
		...
		if (empty($message_strip)) {
			$this->core->js_notify($this->lng['com_msg_incorrect']);
		}
