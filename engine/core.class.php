...
class core{
	...
	public function js_notify($message='', $title='', $type = false, $data = array()) {
		if (empty($title)) { $title = $this->l10n->gettext('error_message'); }
	...
	public function pagination($res=10, $page='', $count=0, $theme='') {
		...
		$start = $pid * $res - $res;
		if ($page === 0 && $count === 0) { return $start; }

		$max = intval(ceil($count / $res));

		if ($pid<=0 || $pid>$max) { return null; }
	...
	public function load_mode($mode){
		...
		if(!method_exists($module, "content")){ return sprintf($this->l10n->gettext('method_not_found'), 'content'); }
	...
	public function load_def_blocks($is_all =true, $block = '') {
		...
		if ($is_all) {
			...
			if(empty($block_config)){ return false; }
			...
			foreach($block_config as $file){
				...
				if (!method_exists($block_class, 'content')) { unset($block_class); continue; }
			...
			if (file_exists($block_config)) { include($block_config); }

			if ($cfg['ENABLE']) {
				if (file_exists($block_file_class)) { include($block_file_class); }
	...
	private function search_array($active = 'news'){
		if(empty($this->cfg->search)){ return null; }
		ob_start();
	...
	public function search(){
		if(!$this->is_access('sys_search')){ return null; }
