...
class module
{
	...
	public function content()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			if (phpversion() < 5.5) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_php_version'], 'install/');
			}

			if (@ini_get('register_globals') == 'off') {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_register_globals'], 'install/');
			}

			if (@ini_get('allow_url_fopen') == '0' || @ini_get('allow_url_fopen') == 'false') {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_fopen'], 'install/');
			}

			if (!class_exists('Locale')) {
				$this->install->notify($this->lng['e_msg'], 'l10n', 'install/');
			}

			if (!function_exists('ImageCreateFromJpeg')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_gd'], 'install/');
			}

			if (!function_exists('mysql_query') && !function_exists('mysqli_query')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_mysql_not_found'], 'install/');
			}

			if (!function_exists('ob_start')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_buffer'], 'install/');
			}

			if (!$this->check_write_all(DIR_ROOT . 'configs')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_configs'], 'install/');
			}

			if (!$this->check_write_all(DIR_ROOT . 'configs/modules')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_modules'], 'install/');
			}
			...
			if (!$this->check_write_all(DIR_ROOT . 'data')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_cache'], 'install/');
			}

			if (!is_writable(DIR_ROOT . 'uploads') || !is_readable(DIR_ROOT . 'uploads')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_uploads'], 'install/');
			}

			if (!is_writable(DIR_ROOT . $this->cfg['main']['cloak_path']) || !is_readable(DIR_ROOT . $this->cfg['main']['cloak_path'])) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_cloaks'], 'install/');
			}

			if (!is_writable(DIR_ROOT . 'uploads/panel-icons') || !is_readable(DIR_ROOT . 'uploads/panel-icons')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_icons'], 'install/');
			}

			if (!is_writable(DIR_ROOT . $this->cfg['main']['skin_path']) || !is_readable(DIR_ROOT . $this->cfg['main']['skin_path'])) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_skins'], 'install/');
			}

			if (!is_writable(DIR_ROOT . $this->cfg['main']['skin_path'] . 'interface') || !is_readable(DIR_ROOT . $this->cfg['main']['skin_path'] . 'interface')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_intf'], 'install/');
			}

			if (!is_writable(DIR_ROOT . 'uploads/smiles') || !is_readable(DIR_ROOT . 'uploads/smiles')) {
				$this->install->notify($this->lng['e_msg'], $this->lng['e_perm_smiles'], 'install/');
			}
