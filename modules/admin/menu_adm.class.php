...
class submodule
{
	...
	private function add()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if (!$this->db->query("INSERT INTO `mcr_menu_adm` (title, gid, `text`, `url`, `target`, `access`, `priority`, icon)
				VALUES ('$title', '$gid', '$text', '$url', '$target', '$permissions', '$priority', '$icon')")) {
				$this->core->notify($this->l10n->gettext('error_message'), $this->l10n->gettext('error_sql_critical'), 2, '?mode=admin&do=menu_adm');
			}
	...
	private function edit()
	{
		...
		$data = [
			"PAGE" => $this->l10n->gettext('mcp_edit_page_name'),
			"TITLE" => $this->db->HSC($ar['title']),
