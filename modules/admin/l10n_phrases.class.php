...
class submodule
{
	...
	public function __construct(core $core)
	{
		...
		if (!$this->core->is_access('sys_adm_l10n')) {
			$this->core->notify('403');
		}
		...
		$this->core->header .= $this->core->sp(MCR_THEME_MOD . "admin/l10n/phrases/header.phtml");
	}
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			switch ($post['language']) {
				case 0:
                    ...
                    ";
					if (!$this->db->query($add_master_phrase)) {
						$this->core->notify(
							$this->l10n->gettext('error_message'),
	...
	protected function update_child_languages($parent, $new_phrase, $action = false)
	{
		switch ($parent) {
			case 0:
				$languages = "SELECT `id`, `language`, `phrases` FROM `mcr_l10n_languages`";
				break;
			default :
				$languages = "SELECT `id`, `language`, `phrases` FROM `mcr_l10n_languages` WHERE `parent_language`='{$parent}'";
				break;
		}
	...
	protected function update_language($sql_to_get, $new_phrase, $action = false)
	{
		...
		if (!$data_lang || $this->db->num_rows($data_lang) <= 0) {
			return null;
		};
		...
		foreach ($data_lang as $language) {
			...
			if ($action == 'add' && array_key_exists(strval($new_phrase['key']), $phrases)) {
				$this->core->notify(
					$this->l10n->gettext('error_message'),
            ...
            ";
			if (!$this->db->query($update_phrases)) {
				$this->core->notify(
					$this->l10n->gettext('error_message'),
	...
	protected function languages($select_language = -1)
	{
		...
		while ($language = $this->core->db->fetch_assoc($languages)) {
			...
			echo $this->core->sp(MCR_THEME_MOD . "admin/l10n/phrases/languages.phtml", $data);
		}
	...
	protected function delete_child_phrases($parent, $phrases)
	{
		switch ($parent) {
			case 0:
				$children = "SELECT `id`, `phrases`, `language` FROM `mcr_l10n_languages`";
				break;
			default:
				$children = "SELECT `id`, `phrases`, `language` FROM `mcr_l10n_languages` WHERE `parent_language`='{$parent}'";
				break;
		}
	...
	protected function delete_phrases($children, $phrases)
	{
		...
		if (!$child_languages || $this->db->num_rows($child_languages) <= 0) {
			return null;
		};
	...
	protected function all_phrases($language)
	{
		...
		if (!$master) {
			...
			$locale = '<span style="color:#d9534f;"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i>' . Locale::getDefault() . ' (master)</span>';
		}
