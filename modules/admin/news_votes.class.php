...
class submodule
{
	...
	public function __construct(core $core)
	{
		...
		if (!$this->core->is_access('sys_adm_news_votes')) {
	...
	private function delete()
	{
		...
		if (empty($list)) {
			$this->core->notify($this->l10n->gettext("e_msg"), $this->l10n->gettext('nvt_not_selected'), 2, '?mode=admin&do=news_votes');
		}
	...
	private function votes_array()
	{
		...
		if (!$query || $this->db->num_rows($query) <= 0) {
			return $this->core->sp(MCR_THEME_MOD . "admin/news_votes/vote-none.phtml");
		}
