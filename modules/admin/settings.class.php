...
class submodule
{
	...
	private function pagin()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($cfg_keys !== $post_keys) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_e_hash'), 2, '?mode=admin&do=settings&op=pagin');
			}

			$cfg = $this->to_int_keys($post);

			if (!$this->cfg->savecfg($cfg, 'pagin.php', 'pagin')) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_e_cfg_save'), 2, '?mode=admin&do=settings&op=pagin');
			}
	...
	private function _mail()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if (!$this->cfg->savecfg($cfg, 'mail.php', 'mail')) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_e_cfg_save'), 2, '?mode=admin&do=settings&op=mail');
			}
	...
	private function search()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($_POST['key']) || !isset($cfg[$_POST['key']])) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('error_hack'), 2, '?mode=admin&do=settings&op=search');
			}

			if (!$this->core->validate_perm(@$_POST['permissions'])) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('error_hack'), 2, '?mode=admin&do=settings&op=search');
			}
			...
			if (!$this->cfg->savecfg($cfg, 'search.php', 'search')) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_e_cfg_save'), 2, '?mode=admin&do=settings&op=search');
			}
	...
	private function base()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if (!$this->cfg->savecfg($db, 'db.php', 'db')) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_e_cfg_save'), 2, '?mode=admin&do=settings&op=base');
			}
	...
	private function functions()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if (!$this->cfg->savecfg($cfg, 'functions.php', 'func')) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_e_cfg_save'), 2, '?mode=admin&do=settings&op=functions');
			}
	...
	private function main()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if (!$this->is_theme_exist($s_theme)) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_theme_incorrect'), 2, '?mode=admin&do=settings');
			}
			$cfg['s_theme'] = $s_theme;
			...
			if (!$this->is_captcha_exist($captcha)) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_captcha_incorrect'), 2, '?mode=admin&do=settings');
			}
			$cfg['captcha'] = $captcha;
			...
			if (!$this->cfg->savecfg($cfg)) {
				$this->core->notify($this->l10n->gettext('error_msg'), $this->l10n->gettext('set_e_cfg_save'), 2, '?mode=admin&do=settings');
			}
	...
	private function is_theme_exist($var)
	{
		...
		if (!file_exists(MCR_ROOT . 'themes/' . $var . '/theme.php')) {
			return false;
		}
	...
	private function themes($select = '')
	{
		...
		foreach ($scan as $key => $value) {
			if ($value == '.' || $value == '..' || !is_dir(MCR_ROOT . 'themes/' . $value)) {
				continue;
			}

			if (!file_exists(MCR_ROOT . 'themes/' . $value . '/theme.php')) {
				continue;
			}
