...
class submodule
{
	...
	public function content()
	{
		...
		if (!$query) {
			$this->core->js_notify($this->core->l10n->gettext('error_sql_critical'));
		}
		...
		if (!$this->db->remove_fast('mcr_online', "`date_update`<'$expire'")) {
			$this->core->js_notify($this->core->l10n->gettext('error_sql_critical'));
		}

		if ($this->user->is_auth) {
			if (!$this->db->remove_fast('mcr_online', "`ip`='{$this->user->ip}'")) {
				$this->core->js_notify($this->core->l10n->gettext('error_sql_critical'));
			}
		}

		$this->core->js_notify('Поздравляем!', $this->core->l10n->gettext('error_success'), true);
	}
}
