...
class module
{
	...
	public function content()
	{
		if ($this->user->is_auth) {
			$this->core->notify($this->core->lng['e_msg'], $this->core->lng['e_403'], 2, '?mode=403');
		}

		if (!$this->core->is_access('sys_restore')) {
			$this->core->notify($this->core->lng['e_msg'], $this->lng['e_perm'], 1, "?mode=403");
		}
	...
	private function accept()
	{
		...
		if (!$query || $this->db->num_rows($query) <= 0) {
			$this->core->notify($this->core->lng['e_attention'], $this->core->lng['e_sql_critical'], 1, "?mode=restore");
		}
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			$update = $this->db->query("
				...
				SET
					...
					`{$us_f['salt']}`='$salt',
					`{$us_f['tmp']}`='$tmp',
					`{$us_f['ip_last']}`='{$this->user->ip}',
					`{$us_f['date_last']}`='" . $time->format('Y-m-d H:i:s') . "'
				WHERE `{$us_f['id']}`='$uid'
			");
	...
	private function send()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($_SESSION['m_send_id'] > 5) {
				$this->core->notify($this->core->lng['e_msg'], $this->lng['e_limit'], 1, "?mode=restore");
			}

			$email = $this->db->safesql(@$_POST['email']);

			if (empty($email)) {
				$this->core->notify($this->core->lng['e_msg'], $this->lng['invalid_email'], 1, "?mode=restore");
			}
			...
			if (!$query || $this->db->num_rows($query) <= 0) {
				$this->core->notify($this->core->lng['e_msg'], $this->lng['email_not_found'], 1, "?mode=restore");
			}
			...
			if (!$this->core->send_mail($email, $this->lng['email_title'], $message)) {
				$this->core->notify($this->core->lng['e_msg'], $this->core->lng['e_critical'], 1, "?mode=restore");
			}
